<template>
   <article aeris-task-display-component-host>
      <div v-show="!editMode">
         ...
         <aside>
            <input v-model="task.done" :id="task.id" type="checkbox" @change="taskDoneChanged">
            <label :for="task.id"/>            
            <div>
      ...
      <div v-show="editMode">
         <task-edit-component :edited-title="task.title" :edited-id="task.id" @close="closeEdit"/>
      </div>
   </article>
</template>
<script>
export default {
  name: "task-display-vue-component",
  ...
  created: function() {
    ...
    console.log("task vue component created");
  },
  methods: {
    taskDoneChanged() {
    ...
    save() {
      this.$http
        .put("http://localhost:8080/todo/edit/" + this.task.id, this.task)
        .then(
          response => {
    ...
    deleteTask() {
      this.$http
        .delete("http://localhost:8080/todo/delete/" + this.task.id)
        .then(
          response => {
